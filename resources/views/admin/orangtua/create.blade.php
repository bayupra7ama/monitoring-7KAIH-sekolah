{{-- File: resources/views/admin/orangtua/create.blade.php --}}
@extends('layouts.admin')

@section('content')
<div class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow">
    <h2 class="text-xl font-bold mb-4">Tambah Orang Tua Baru</h2>

    {{-- ALERT ERROR --}}
    @if ($errors->any())
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
            <strong class="font-bold">Gagal Menambahkan!</strong>
    ...
    <form action="{{ route('orangtua.store') }}" method="POST" id="parentForm">
        @csrf
        
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2">Nama Lengkap</label>
            <input type="text" name="name" value="{{ old('name') }}" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-indigo-300" placeholder="Nama Orang Tua" required>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2">Email (Opsional)</label>
            <input type="email" name="email" value="{{ old('email') }}" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-indigo-300" placeholder="Boleh dikosongkan">
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2">No. WhatsApp (Login)</label>
            <input type="number" name="phone" value="{{ old('phone') }}" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-indigo-300" placeholder="08..." required>
            @error('phone')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>

        {{-- PILIH ANAK (Custom Search: Nama & NISN) --}}
...
<script>
    ...
    function renderChips() {
        ...
        selectedStudents.forEach(student => {
            ...
            // Tampilkan Nama dan NISN di Chip
            chip.innerHTML = `
    ...
    searchInput.addEventListener('input', function(e) {
        ...
        // Search Logic: Cek Nama ATAU Cek NISN
        ...
        if (filtered.length > 0) {
            ...
            filtered.forEach(s => {
                ...
                // Tampilan Dropdown: Nama (NISN) - Kelas
